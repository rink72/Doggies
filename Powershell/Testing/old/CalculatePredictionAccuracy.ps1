...
$RaceQuery = "SELECT RACEID, WINPAYING, PLACEPAYING, QUINELLAPAYING, TRIFECTAPAYING, FIRST4PAYING FROM RACES WHERE RACEID in ( SELECT DISTINCT RACEID FROM PREDICTEDRESULTS) ORDER BY newID()"
...
foreach($race in $RaceDetails)
{
    $count
    $raceID = $race.RACEID
    $predictQuery = "SELECT TOP 4 [DOGID], [PREDICTEDRESULTS] FROM PREDICTEDRESULTS WHERE RACEID = $RaceID ORDER BY PREDICTEDRESULTS"
    #$predictQuery = "SELECT TOP 4 [DOGID], [PREDICTEDRESULTS] FROM PREDICTEDRESULTS WHERE RACEID = $RaceID AND (SELECT TOP 1 PREDICTEDRESULTS FROM PREDICTEDRESULTS WHERE RACEID = $RaceID ORDER BY PREDICTEDRESULTS) < 0 ORDER BY PREDICTEDRESULTS"
    #$predictQuery = "SELECT TOP 4 [DOGID], [PREDICTEDRESULTS] FROM PREDICTEDRESULTS WHERE RACEID = $RaceID AND (SELECT STDEV(PREDICTEDRESULTS) FROM PREDICTEDRESULTS WHERE RACEID = $RaceID) > 2.5"
