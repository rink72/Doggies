...
function ProcessMeet($URL)
{
    ...
    foreach($race in $races)
    {
        ...
        $RaceQuery = "EXEC AddFutureRaceDetails @TRACKID = $trackid,@DATE = '$RaceDate',@DISTANCE = $raceDistance,@WINPAYING = $WinReturn,@PLACEPAYING = $Place1Return,@QUINELLAPAYING = $Quinella,@TRIFECTAPAYING = $Trifecta,@FIRST4PAYING = $First4,@TRACKCONDITION = '$trackcond',@WEATHER = '$weather',@RACENUMBER = $RaceNum"
        ...
        foreach($dog in $Dogs)
        {
            ...
            $SQLQueries += "EXEC AddFutureRaceResult @RACEID=$RaceID,@TRACKID=$TrackID,@DOGID=$DogID,@BOX=$box,@RESULT='$position'"
        }
